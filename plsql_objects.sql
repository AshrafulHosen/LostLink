...
CREATE OR REPLACE FUNCTION FN_GET_OVERALL_STATS RETURN SYS_REFCURSOR AS
    v_cursor SYS_REFCURSOR;
BEGIN
    OPEN v_cursor FOR
    SELECT
        ...
        (SELECT COUNT(*) FROM CLAIMS WHERE STATUS = 'APPROVED') AS TOTAL_RECOVERED,
        (SELECT ROUND(AVG(C.CREATED_AT - L.CREATED_AT), 1) FROM CLAIMS C JOIN LOST_ITEMS L ON C.CLAIMANT_ID = L.USER_ID WHERE C.STATUS = 'APPROVED') AS AVG_RESOLUTION_DAYS,
        (SELECT ROUND(AVG(MATCH_SCORE), 0) FROM MATCHES WHERE MATCH_SCORE >= 75) AS AVG_HIGH_MATCH_ACCURACY
    FROM DUAL;
    RETURN v_cursor;
END;
/

CREATE OR REPLACE FUNCTION FN_GET_CATEGORY_STATS RETURN SYS_REFCURSOR AS
    v_cursor SYS_REFCURSOR;
BEGIN
    OPEN v_cursor FOR
...
CREATE OR REPLACE FUNCTION FN_GET_HOTSPOT_STATS RETURN SYS_REFCURSOR AS
    v_cursor SYS_REFCURSOR;
BEGIN
    OPEN v_cursor FOR
    SELECT * FROM (
        SELECT LOC AS LOCATION, COUNT(*) AS ITEM_COUNT
        FROM (
            SELECT LOST_LOCATION AS LOC FROM LOST_ITEMS
            UNION ALL
            SELECT FOUND_LOCATION AS LOC FROM FOUND_ITEMS
        )
        GROUP BY LOC
        ORDER BY ITEM_COUNT DESC
    ) WHERE ROWNUM <= 5;
...
CREATE OR REPLACE FUNCTION FN_GET_RECENT_ACTIVITY RETURN SYS_REFCURSOR AS
    v_cursor SYS_REFCURSOR;
BEGIN
    OPEN v_cursor FOR
    SELECT * FROM (
        SELECT 'LOST' AS ITEM_TYPE, LOST_ID AS ITEM_ID, ITEM_NAME, CATEGORY, COLOR, LOST_LOCATION AS LOCATION, LOST_DATE AS ITEM_DATE, STATUS
        FROM LOST_ITEMS
        UNION ALL
        SELECT 'FOUND' AS ITEM_TYPE, FOUND_ID AS ITEM_ID, ITEM_NAME, CATEGORY, COLOR, FOUND_LOCATION AS LOCATION, FOUND_DATE AS ITEM_DATE, STATUS
        FROM FOUND_ITEMS
