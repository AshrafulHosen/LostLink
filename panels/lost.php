...
SELECT
    ...
    LOST_DATE,
    STATUS,
    FN_GET_ITEM_IMAGE(LOST_ID, NULL) AS IMAGE_PATH
FROM LOST_ITEMS
...
<div class="panel" id="panel-lost">
  ...
  <div class="section-head">
    ...
    <button class="btn btn-primary" onclick="showPanel('report', document.querySelector('.nav-item:nth-child(9)'))">
    ...
    <table>
      ...
      <?php foreach($myItems as $row): ?>
        <tr data-category="<?php echo strtolower(htmlspecialchars($row['CATEGORY'])); ?>" data-status="<?php echo strtolower(htmlspecialchars($row['STATUS'])); ?>">
          <td>L-<?php echo str_pad($row['LOST_ID'],3,'0',STR_PAD_LEFT); ?></td>
          <td>
            <div class="td-main">
          ...
          <td>
            ...
            elseif($status === 'Recovered') echo '<span class="badge badge-recovered">' . $status . '</span>';
  ...
  <div style="font-size:12px;color:var(--txt2);margin-bottom:12px;text-transform:uppercase;letter-spacing:1px;">Other Lost Items</div>
  <?php endif; ?>
